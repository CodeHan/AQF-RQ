...
def parse_args():
    ...
    parser.add_argument("--fp32", type=str2bool, default=False)
    parser.add_argument("--real_sample", type=str2bool, default=False)
    ...
    parser.add_argument("--specified_device",type=int,default=-1)
    parser.add_argument("--pretrain_path",type=str,required=True)

    parser.add_argument('--restore',type=str2bool,default=False)
    parser.add_argument('--restore_task', type=str, default='')
    parser.add_argument('--restore_epoch', type=str, default='')
    parser.add_argument('--teacher_epoch', type=str, default='')
    parser.add_argument('--restore_inplace', type=str2bool, default=False)
    parser.add_argument('--retrain',type=str2bool,default=False)
    parser.add_argument('--swap_cq', type=str2bool, default=True)
    parser.add_argument('--train_question', type=str2bool, default=True)
    parser.add_argument("--eta", type=float, default=0.25)
    parser.add_argument('--only_extra', type=str2bool, default=True)
    ...
    args.lr_adapt=args.learning_rate*10
    if args.skip_tasks and args.skip_tasks[0] not in args.tasks:
        args.skip_tasks = None
    if args.train_question:
        args.swap_cq=True

    #if args.seq_train_type=='gem':
        #args.add_task_tokens=False
    args.model_dir_root_base=args.model_dir_root
    ...
    args.model_dir_root = os.path.join(args.model_dir_root, args.model_name,
            args.seq_train_type, "{}_{}{}{}{}".format("_".join(args.tasks),
                ...
                '_swap' if args.swap_cq else '',
                '_ques%s%s'%('' if args.eta == 0.25 else args.eta,'_extra' if args.only_extra else '') if args.train_question else '',
                '_gen' if not args.add_task_tokens else '') if "lll" in args.seq_train_type else "_".join(args.tasks))

    args.device_ids = GPUtil.getAvailable(order='memory',maxLoad=0.99, maxMemory=0.99, limit=args.n_gpus) if args.specified_device<0 else [args.specified_device]
    ...
    if not args.fp32:
        ...
        MEMORY_FACTOR = dict([k, v*1.1] for k, v in MEMORY_FACTOR.items())
    ...
    model_config = config_class.from_pretrained(args.pretrain_path)
