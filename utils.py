...
def get_loss_dict(qa_loss, lm_loss=torch.tensor(0.),q_lm_loss=torch.tensor(0.)):
    loss_dict={}
    loss_dict['qa_loss']=qa_loss
    loss_dict['lm_loss'] = lm_loss
    loss_dict['q_lm_loss'] = q_lm_loss

    return loss_dict

def filter_special_token(question,is_extra,filter_str='goodbye'):
    filter_str_id = set(TOKENIZER.encode(filter_str))
    train_q_mask=[]
    for i in range(len(is_extra)):
        if is_extra[i]==1:
            if len(filter_str_id.intersection(question[i].detach().cpu()))>0:
                train_q_mask.append(False)
            else:
                train_q_mask.append(True)
        else:
            train_q_mask.append(False)
    return train_q_mask
...
def get_losses_q(parallel_model, cqa, Y, gen_X, gen_Y, gen_X_q, gen_Y_q, is_extra,loss_fct):
    if "lll" in args.seq_train_type:
        qa_logits = parallel_model(cqa)
        lm_logits = parallel_model(gen_X)

        mask = [_is_extra == 1 for _is_extra in is_extra]
        extra_num = sum([l.sum() for l in is_extra])
        if args.only_extra:
            if extra_num > 0:
                gen_X_q_extra = [(torch.masked_select(_gen_X_q[0], _mask.unsqueeze(-1)).reshape([-1,_gen_X_q[0].shape[-1]]),) for _gen_X_q, _mask in zip(gen_X_q, mask)]
                gen_Y_q_extra = [torch.masked_select(_gen_Y_q, _mask.unsqueeze(-1)).reshape([-1,_gen_Y_q.shape[-1]]) for _gen_Y_q, _mask in zip(gen_Y_q, mask)]
                q_lm_logits = parallel_model(gen_X_q_extra)
                q_lm_loss = loss_fct([torch.transpose(l, 1, 2) for l in q_lm_logits], gen_Y_q_extra)
            else:
                q_lm_loss = torch.tensor(0.)
        else:
            q_lm_logits = parallel_model(gen_X_q)
            q_lm_loss = loss_fct([torch.transpose(l, 1, 2) for l in q_lm_logits], gen_Y_q)
        ...
        return get_loss_dict(torch.mean(qa_loss), lm_loss=args.lm_lambda * torch.mean(lm_loss), q_lm_loss=args.eta * torch.mean(q_lm_loss))
    else:
        qa_logits = parallel_model(cqa)
        qa_loss = loss_fct([torch.transpose(l, 1, 2) for l in qa_logits], Y)
        return get_loss_dict(torch.mean(qa_loss))
...
def get_losses(parallel_model, cqa, Y, gen_X, gen_Y, loss_fct):
    if "lll" in args.seq_train_type:
        ...
        qa_loss = loss_fct([torch.transpose(l, 1, 2) for l in qa_logits], Y)
        lm_loss = loss_fct([torch.transpose(l, 1, 2) for l in lm_logits], gen_Y)
        return get_loss_dict(torch.mean(qa_loss), lm_loss=args.lm_lambda * torch.mean(lm_loss))
    else:
        qa_logits = parallel_model(cqa)
        qa_loss = loss_fct([torch.transpose(l, 1, 2) for l in qa_logits], Y)
        return get_loss_dict(torch.mean(qa_loss))

def get_question_losses(parallel_model, gen_X, gen_Y, loss_fct):
    q_logits = parallel_model(gen_X)
    #print(q_logits[0].shape,gen_Y[0].shape)
    q_loss = loss_fct([torch.transpose(l, 1, 2) for l in q_logits], gen_Y)
...
def varlen_collate_fn(data):
    ...
    gen_X_qs = torch.tensor(pad_all_to_max_len([datum[9] for datum in data], SPECIAL_TOKEN_IDS["pad_token"])).split(batch_size)
    gen_Y_qs = torch.tensor(pad_all_to_max_len([datum[10] for datum in data], FILL_VAL)).split(batch_size)
    is_extra = torch.tensor([datum[-1] for datum in data]).split(batch_size)

    return list(cqs), list(len_cqs), list(cqas), list(len_cqas), list(Ys), list(gen_Xs), list(gen_Ys),list(gen_X_qs),list(gen_Y_qs),list(is_extra)
...
def dynamic_collate_fn(data, batch_size):
    ...
    def local_collate():
        null_counter = 0
        _cqs, _len_cqs, _cqas, _len_cqas, _Ys, _gen_Xs, _gen_Ys,_gen_X_qs,_gen_Y_qs, _is_extra = [], [], [], [], [], [], [], [], [], []
        Y_max_len = max(len(data[j][4]) for j in range(st, ed))
        cq_max_len = max(len(data[j][0]) for j in range(st, ed))
        q_max_len = max(len(data[j][9]) for j in range(st, ed))
        for j in range(st, ed):
            ...
            _gen_X_qs.append(pad_to_max_len(data[j][9], q_max_len - len(data[j][9]), SPECIAL_TOKEN_IDS["pad_token"]))
            _gen_Y_qs.append(pad_to_max_len(data[j][10], q_max_len - len(data[j][10]), FILL_VAL))
        ...
        gen_X_qs.append(torch.tensor(_gen_X_qs))
        gen_Y_qs.append(torch.tensor(_gen_Y_qs))
        is_extra.append(torch.tensor(_is_extra))

    cqs, len_cqs, cqas, len_cqas, Ys, gen_Xs, gen_Ys, gen_X_qs, gen_Y_qs, is_extra = [], [], [], [], [], [], [], [], [], []
    ...
    return cqs, len_cqs, cqas, len_cqas, Ys, gen_Xs, gen_Ys, gen_X_qs, gen_Y_qs,is_extra
...
class QADataset(Dataset):
    def __init__(self, data_paths, data_type, gen_token, extra_data=[],only_question=False):
        ...
        #print('QADataset: gen_token=',gen_token)
        ...
        if len(extra_data) > 0:
            if self.only_question:
                extra_data = map(lambda x: self.etl_single_extra_data_for_question(x), extra_data)
            else:
    ...
    def etl_single_extra_data_for_question(self, data):
        
        gen_token = data[0]
        question=data[1:]
        uid = uuid.uuid1().hex
        data = self.parse_example_for_question(gen_token,question,uid)
    ...
    def parse_example_for_question(self, gen_token, question, idx):
        
        cq_example = self.concat_example([], question, [], [], [], [], [])
        cqa_example = self.concat_example([], question, [], [], [], [], [])
        Y_example = self.concat_example([], [], [], [], [], [], [self.eos_token])
        Y_example = [FILL_VAL] * (len(cqa_example) - len(Y_example)) + Y_example

        gen_X_example = self.concat_example([gen_token], question, [], [], [], [], [])
        gen_Y_example = self.concat_example([], question, [], [], [], [], [FILL_VAL])



        gen_X_question = self.concat_example([gen_token], question, [], [], [], [], [])
        gen_Y_question = self.concat_example([], question, [], [], [], [], [FILL_VAL])

        return cq_example, len(cq_example), cqa_example, len(cqa_example), Y_example, gen_X_example, gen_Y_example, gen_X_question, gen_Y_question,idx
    ...
    def parse_example(self, gen_token, context, question, answer, idx):
        ...
        gen_X_question,gen_Y_question=[-1],[-1]
        if args.train_question:
            gen_X_question = self.concat_example([gen_token], context, [], [], [], [], [])
            gen_Y_question = self.concat_example([], context, [], [], [], [], [FILL_VAL])
...
def check_sample(data):
    c = Counter(data)
    if c[SPECIAL_TOKEN_IDS['ans_token']]!=1:
        return False
    max_values = max(c.values())
    if max_values>7:
        return False
    return True
...
def create_extra_data(task, prev_task, model, train_extra_data,ignore_exist=False):
    if args.real_sample:
    ...
    if os.path.exists(gen_path) and not args.retrain and not ignore_exist:
