...
def test_one_to_one(task_load, task_eval, model, score_dict):
    ...
    cnt = 0
    for n_steps, (cqs, len_cqs, _, _, _, _, _, _, _, _) in enumerate(test_dataloader):
        # assume n_gpus == 1
        cqs = cqs[0]
        len_cqs = len_cqs[0]
    ...
    for i in range(len(test_qadata)):
        _, len_cq, _, _, Y, _, _, _, _, _, _ = test_qadata[i]
...
def test_one_to_many(task_load):
    ...
    for ep in range(args.n_train_epochs[task_load]):
        ...
        if not os.path.exists(os.path.join(model_dir, 'model-{}'.format(ep+1))) and args.tasks.index(task_load) == 0:
            model_dir = get_model_dir([task_load], True)
            logger.info(
                '%s is not found, we load default model from %s' % (os.path.join(model_dir, 'model-{}'.format(ep)),
                                                                   model_dir))
...
if __name__ == '__main__':
    ...
    seed_bias=[int(1000*random.random()) for _ in range(args.multi_run)] if args.multi_run>1 else [0]
